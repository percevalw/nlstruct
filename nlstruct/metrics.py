...
class PrecisionRecallF1Metric(Metric):
    ...
    def update(self, preds, target):
        ...
        def rec(p, g):
            if (len(p) and isinstance(p[0], tuple)) or (
                len(g) and isinstance(g[0], tuple)
            ):
    ...
    def compute(self):
        ...
        return {
            ...
            self.prefix
            + "f1": (self.true_positive * 2) / (self.pred_count + self.gold_count),
        }
...
class DocumentEntityMetric(Metric):
    def __init__(
        ...
        self.binarize_label_threshold = (
            float(binarize_label_threshold)
            if binarize_label_threshold is not False
            else binarize_label_threshold
        )
        self.binarize_tag_threshold = (
            float(binarize_tag_threshold)
            if binarize_tag_threshold is not False
            else binarize_tag_threshold
        )
        self.add_state("true_positive", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("pred_count", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("gold_count", default=torch.tensor(0.0), dist_reduce_fx="sum")
    ...
    def compare_two_samples(self, pred_doc, gold_doc, return_match_scores=False):
        ...
        if self.explode_fragments:
            pred_doc_entities = [
                {"label": f.get("label", "main"), "fragments": [f]}
                for f in dedup(
                    (f for entity in pred_doc_entities for f in entity["fragments"]),
                    key=lambda x: (x["begin"], x["end"], x.get("label", None)),
                )
            ]
            gold_doc_entities = [
                {"label": f.get("label", "main"), "fragments": [f]}
                for f in dedup(
                    (f for entity in gold_doc_entities for f in entity["fragments"]),
                    key=lambda x: (x["begin"], x["end"], x.get("label", None)),
                )
            ]

        words = regex_tokenize(
            gold_doc["text"],
            reg=self.word_regex,
            do_unidecode=True,
        ...
        for entity in pred_doc_entities:
            ...
            for fragment in entity["fragments"]:
                ...
                all_fragment_labels.add(
                    fragment["label"] if self.eval_fragments_label else "main"
                )
        ...
        fragments_begin, fragments_end = split_spans(
            fragments_begin, fragments_end, words["begin"], words["end"]
        )
        pred_entities_labels = [[False] * len(all_entity_labels)] * max(
            len(pred_doc_entities), 1
        )
        pred_tags = [
            [[False] * len(words["begin"]) for _ in range(len(all_fragment_labels))]
            for _ in range(max(len(pred_doc_entities), 1))
        ]  # n_entities * n_token_labels * n_tokens
        gold_entities_labels = [[False] * len(all_entity_labels)] * max(
            len(gold_doc_entities), 1
        )
        gold_entities_optional_labels = [[False] * len(all_entity_labels)] * max(
            len(gold_doc_entities), 1
        )
        gold_tags = [
            [[False] * len(words["begin"]) for _ in range(len(all_fragment_labels))]
            for _ in range(max(len(gold_doc_entities), 1))
        ]  # n_entities * n_token_labels * n_tokens

        for entity_idx, (entity_fragments, entity) in enumerate(
            zip(pred_entities_fragments, pred_doc_entities)
        ):
            for fragment_idx, fragment in zip(entity_fragments, entity["fragments"]):
                ...
                label = all_fragment_labels.index(
                    fragment["label"] if self.eval_fragments_label else "main"
                )
            ...
            # *(("{}:{}".format(att["name"], att["label"]) for att in entity["attributes"]) if self.eval_attributes else ())]
            pred_entities_labels[entity_idx] = [
                label in entity_labels for label in all_entity_labels
            ]

        for entity_idx, (entity_fragments, entity) in enumerate(
            zip(gold_entities_fragments, gold_doc_entities)
        ):
            for fragment_idx, fragment in zip(entity_fragments, entity["fragments"]):
                begin = fragments_begin[fragment_idx]
                end = fragments_end[fragment_idx]
            ...
            # *(("{}:{}".format(att["name"], att["value"]) for att in entity["attributes"]) if self.eval_attributes else ())]
            gold_entities_labels[entity_idx] = [
                label in entity_labels for label in all_entity_labels
            ]
            gold_entities_optional_labels[entity_idx] = [
                label in entity_optional_labels for label in all_entity_labels
            ]
        ...
        # score = 0.
        tag_denom_match_scores = pred_tags.float().sum(-1).sum(-1).unsqueeze(
            1
        ) + gold_tags.float().sum(-1).sum(-1).unsqueeze(0)
        tag_match_scores = (
            2
            * torch.einsum("pkt,gkt->pg", pred_tags.float(), gold_tags.float())
            / tag_denom_match_scores.clamp_min(1)
        )
        ...
        label_match_precision = torch.einsum(
            "pk,gk->pg",
            pred_entities_labels.float(),
            gold_entities_optional_labels.float(),
        ) / pred_entities_labels.float().sum(-1).unsqueeze(1).clamp_min(1.0)
        label_match_recall = torch.einsum(
            "pk,gk->pg", pred_entities_labels.float(), gold_entities_labels.float()
        ) / gold_entities_labels.float().sum(-1).unsqueeze(0).clamp_min(1.0)
        label_match_scores = 2 / (
            1.0 / label_match_precision + 1.0 / label_match_recall
        )
        ...
        for pred_idx in range(match_scores.shape[0]):
            ...
            matched_scores[pred_idx, gold_idx] = max(
                matched_scores[pred_idx, gold_idx], effective_score
            )

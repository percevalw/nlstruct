...
def load_ncbi_disease(_cache=None):
    ...
    raw = raw.astype({"doc_id": "category"})
    mentions = raw[["doc_id", "begin", "end", "label", "category"]].nlstruct.flatten("mention_id", tile_index=True)
    mentions["mention_id"] = (mentions["doc_id"].astype("str") + "-" + mentions["mention_id"].astype(str)).astype("category")
    mentions_label = mentions[["doc_id", "mention_id", "label"]].nlstruct.flatten("label_id", tile_index=True)
    mentions_label["label_id"] = (mentions_label["mention_id"].astype("str") + "-" + mentions_label["label_id"].astype(str)).astype("category")

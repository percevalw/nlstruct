...
def huggingface_tokenize(docs, tokenizer, with_tqdm=False, with_token_spans=True, **kwargs):
    doc_ids = []
    tokens = []
    begins = []
    ends = []
    ...
    if with_token_spans:
        ...
        for doc_id, text in tqdm(zip(docs["doc_id"], docs["text"]), disable=not with_tqdm, total=len(docs), leave=False, desc="Tokenizing"):
            ...
            for encoded_piece in tokenizer.convert_ids_to_tokens(tokenizer.encode(text, **kwargs)):
                ...
                token_idx.append(token_id)
                token_id += 1
        tokens = pd.DataFrame({"doc_id": doc_ids, "token_id": range(len(token_idx)), "token_idx": token_idx, "token": tokens})

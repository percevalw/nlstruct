...
class WordLevelDocumentEntityMetric(Metric):
    def __init__(
          self,
          word_regex='[\\w\']+|[!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~]',
    ...
    def compare_two_samples(self, pred_doc, gold_doc):
        ...
        for entity in pred_doc["entities"]:
            all_entity_labels.update(set(entity["labels"]))
            ...
            for fragment in entity["fragments"]:
                ...
                all_fragment_labels.add(fragment["label"])
        ...
        pred_entities_labels = [0] * len(pred_doc["entities"])
        pred_tags = [[[False] * len(words["begin"]) for _ in range(len(all_fragment_labels))] for _ in pred_doc["entities"]]  # n_entities * n_token_labels * n_tokens
        gold_entities_labels = [0] * len(gold_doc["entities"])
        gold_tags = [[[False] * len(words["begin"]) for _ in range(len(all_fragment_labels))] for _ in gold_doc["entities"]]  # n_entities * n_token_labels * n_tokens
        ...
        for entity_idx, (entity_fragments, entity) in enumerate(zip(pred_entities_fragments, pred_doc["entities"])):
            for fragment_idx, fragment in zip(entity_fragments, entity["fragments"]):
                ...
                label = all_fragment_labels.index(fragment["label"])
                pred_tags[entity_idx][label][begin:end] = [True] * (end - begin)
            pred_entities_labels[entity_idx] = [label in entity["labels"] for label in all_entity_labels]
        ...
        for entity_idx, (entity_fragments, entity) in enumerate(zip(gold_entities_fragments, gold_doc["entities"])):
            ...
            gold_entities_labels[entity_idx] = [label in entity["labels"] for label in all_entity_labels]
        ...
        if self.binarize_tag_threshold:
        ...
        if self.binarize_label_threshold:
        ...
        pred_count, gold_count = match_scores.shape
        ...
        return score, pred_count, gold_count
